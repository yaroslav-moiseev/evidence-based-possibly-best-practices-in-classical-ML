...
«Корреляции» между категориальными признаками оценивалась с помощью **коэффициента неопределенности** (**Thiel U correlation coefficient** ( **Uncertainty** **Coefficient**)), а не коэффициента Крамера, так как при его использовании мы не теряем информацию из-за симметрии **[12,13]**.
...
Хорошим вариантом для сравнения алгоритмов является тест Макнемара, однако, используемый датасет недостаточно велик, а для тестовых данных изначально было выделено только 15%, поэтому не следует выбирать лучший алгоритм на основании теста Макнемара. Для небольших датасетов рекомендуется использовать вложенную кросс-валидиацию или 5x2cvf-тест **[19]**.
...
Вложенная кросс-валидация затратна по времени и её следовало бы использовать вместе с тюнингом гиперпараметров. С применением 5x2cvf-теста возникли проблемы с использованием с моими пайплайнами.

Выбор алгоритма следует делать исходя из бизнес задачи, необходимого порога на Precision Recall кривой, доступности вычислительных ресурсов/необходимого времени прогнозирования.
